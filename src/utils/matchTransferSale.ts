  ...
  if (TransferId && transferAmount && TransactionId && SaleId) {
    ...
    if (transferEntity && transferEntity.amount == constants.BIGDECIMAL_ZERO) {
      ...
      if (transactionEntity) {
        ...
        transactionEntity.unmatchedTransferCount =
          transactionEntity.unmatchedTransferCount - 1;
        ...
        if (
          ...
          if (collectionEntity) {
            ...
            collectionEntity.totalVolume = collectionEntity.totalVolume.plus(
              transferAmount
            );
          ...
          let hourlyCollectionSnapshotEntityId =
            transferEntity.collection + "-" + hour.toString();

          let hourlyCollectionSnapshotEntity = hourlyCollectionSnapshot.load(
            hourlyCollectionSnapshotEntityId
          );

          if (!hourlyCollectionSnapshotEntity) {
            hourlyCollectionSnapshotEntity = new hourlyCollectionSnapshot(
              hourlyCollectionSnapshotEntityId
            );
            hourlyCollectionSnapshotEntity.timestamp = hour * 3600;
            hourlyCollectionSnapshotEntity.collection =
              transferEntity.collection;
            hourlyCollectionSnapshotEntity.hourlyVolume =
              constants.BIGDECIMAL_ZERO;
            hourlyCollectionSnapshotEntity.hourlyTransactions = 0;
            hourlyCollectionSnapshotEntity.hourlyAvgSale =
              constants.BIGDECIMAL_ZERO;
            hourlyCollectionSnapshotEntity.topSale = constants.BIGDECIMAL_ZERO;
            hourlyCollectionSnapshotEntity.bottomSale =
              constants.BIGDECIMAL_ZERO;
          ...
          hourlyCollectionSnapshotEntity.hourlyVolume = hourlyCollectionSnapshotEntity.hourlyVolume.plus(
            transferAmount
          );
          ...
          hourlyCollectionSnapshotEntity.hourlyTransactions =
            hourlyCollectionSnapshotEntity.hourlyTransactions + 1;

          // Daily bottom sale
          if (
            transferAmount < hourlyCollectionSnapshotEntity.bottomSale ||
            (hourlyCollectionSnapshotEntity.bottomSale ==
              constants.BIGDECIMAL_ZERO &&
              transferAmount != constants.BIGDECIMAL_ZERO)
          ) {
          ...
          hourlyCollectionSnapshotEntity.hourlyAvgSale = hourlyCollectionSnapshotEntity.hourlyVolume.div(
            BigDecimal.fromString(
              hourlyCollectionSnapshotEntity.hourlyTransactions.toString()
            )
          );
          ...
          // Collection Address - Day
          let dailyCollectionSnapshotEntityId =
            transferEntity.collection + "-" + day.toString();

          let dailyCollectionSnapshotEntity = dailyCollectionSnapshot.load(
            dailyCollectionSnapshotEntityId
          );

          if (!dailyCollectionSnapshotEntity) {
            dailyCollectionSnapshotEntity = new dailyCollectionSnapshot(
              dailyCollectionSnapshotEntityId
            );
            dailyCollectionSnapshotEntity.timestamp = day * 86400;
            dailyCollectionSnapshotEntity.collection =
              transferEntity.collection;
            dailyCollectionSnapshotEntity.dailyVolume =
              constants.BIGDECIMAL_ZERO;
            dailyCollectionSnapshotEntity.dailyAvgSale =
              constants.BIGDECIMAL_ZERO;
            dailyCollectionSnapshotEntity.dailyTransactions = 0;
            dailyCollectionSnapshotEntity.topSale = constants.BIGDECIMAL_ZERO;
            dailyCollectionSnapshotEntity.bottomSale =
              constants.BIGDECIMAL_ZERO;
          ...
          // Updating daily total volume & top sale
          dailyCollectionSnapshotEntity.dailyVolume = dailyCollectionSnapshotEntity.dailyVolume.plus(
            transferAmount
          );
          ...
          // Updating daily total number of transactions
          dailyCollectionSnapshotEntity.dailyTransactions =
            dailyCollectionSnapshotEntity.dailyTransactions + 1;

          // Daily bottom sale
          if (
            transferAmount < dailyCollectionSnapshotEntity.bottomSale ||
            (dailyCollectionSnapshotEntity.bottomSale ==
              constants.BIGDECIMAL_ZERO &&
              transferAmount != constants.BIGDECIMAL_ZERO)
          ) {
          ...
          dailyCollectionSnapshotEntity.dailyAvgSale = dailyCollectionSnapshotEntity.dailyVolume.div(
            BigDecimal.fromString(
              dailyCollectionSnapshotEntity.dailyTransactions.toString()
            )
          );
          ...
          let weeklyCollectionSnapshotEntityId =
            transferEntity.collection + "-" + week.toString();

          let weeklyCollectionSnapshotEntity = weeklyCollectionSnapshot.load(
            weeklyCollectionSnapshotEntityId
          );

          if (!weeklyCollectionSnapshotEntity) {
            weeklyCollectionSnapshotEntity = new weeklyCollectionSnapshot(
              weeklyCollectionSnapshotEntityId
            );
            weeklyCollectionSnapshotEntity.timestamp = week * 604800;
            weeklyCollectionSnapshotEntity.collection =
              transferEntity.collection;
            weeklyCollectionSnapshotEntity.weeklyVolume =
              constants.BIGDECIMAL_ZERO;
            weeklyCollectionSnapshotEntity.weeklyTransactions = 0;
            weeklyCollectionSnapshotEntity.weeklyAvgSale =
              constants.BIGDECIMAL_ZERO;
            weeklyCollectionSnapshotEntity.topSale = constants.BIGDECIMAL_ZERO;
            weeklyCollectionSnapshotEntity.bottomSale =
              constants.BIGDECIMAL_ZERO;
          ...
          weeklyCollectionSnapshotEntity.weeklyVolume = weeklyCollectionSnapshotEntity.weeklyVolume.plus(
            transferAmount
          );
          ...
          weeklyCollectionSnapshotEntity.weeklyTransactions =
            weeklyCollectionSnapshotEntity.weeklyTransactions + 1;

          // Weekly bottom sale
          if (
            transferAmount < weeklyCollectionSnapshotEntity.bottomSale ||
            (weeklyCollectionSnapshotEntity.bottomSale ==
              constants.BIGDECIMAL_ZERO &&
              transferAmount != constants.BIGDECIMAL_ZERO)
          ) {
          ...
          weeklyCollectionSnapshotEntity.weeklyAvgSale = weeklyCollectionSnapshotEntity.weeklyVolume.div(
            BigDecimal.fromString(
              weeklyCollectionSnapshotEntity.weeklyTransactions.toString()
            )
          );
